...
def createNetwork(sequence_length, num_classes, embedding_size, filter_sizes, num_filters, l2_reg_lambda=0.0,num_hidden=100,epoch=100,learning_rate=0.1,batchSize=50,point=0.5,keep_prob=0.5):
    ...
    regularization_cost = l2_reg_lambda / batchSize * (
    tf.reduce_sum(tf.nn.l2_loss(tf.get_collection('l2-reg-conv-lstm'))) + tf.reduce_sum(
        tf.nn.l2_loss(tf.get_collection('l2-reg-conv'))) + tf.reduce_sum(
        tf.nn.l2_loss(tf.get_collection(tf.GraphKeys.WEIGHTS))))
    costFunc = loss + regularization_cost
    optimizer = tf.train.GradientDescentOptimizer(learning_rate=learning_rate, ).minimize(costFunc)
    with tf.Session() as sess:
        for k in range(epoch):
            epochcost = 0
            #----------------------------------------------------------------------------后期修改训练集---------------------------------------------------------------
            for i in range(int(200/batchSize)):
                ...
                if k == 0:
                    sess.run(tf.global_variables_initializer())
                cost,_ = sess.run([costFunc,optimizer],feed_dict={x:x_data,y:y_data,dropout_keep_prob:keep_prob})
                epochcost += cost
...
if __name__ == '__main__':
    createNetwork(sequence_length=200,num_classes=2,embedding_size=200,filter_sizes=5,num_filters=10,l2_reg_lambda=0.1,epoch=20,learning_rate=0.5,batchSize=2)